...
contract CommuteIO {
    ...
    event PassengerRequestApproved(
        uint256 passRequestID,
        string passVehicleDetailsHash
    );
    event PassengerRequestRejected(
        uint256 passRequestID,
        string passVehicleDetailsHash
    );
    ...
    function approvePassengerRequest(
        uint256 _passRequestID /*string memory  _currentDate*/
    ) public {
        require(
            passengerRequests[_passRequestID].passRequestID != 0,
            "Request not found"
        );
        require(
            passengerRequests[_passRequestID].passRequestStatus == 0,
        ...
        emit PassengerRequestApproved(
            _passRequestID,
            passengerRequests[_passRequestID].passVehicleDetailsHash
        );
    }
    ...
    function getPassRequestDetails(
        uint256 _passRequestID
    ) public view returns (PassengerRequest memory) {
        return passengerRequests[_passRequestID];
    }

    function getPassDetails(
        uint256 _passID
    ) public view returns (Passenger memory) {
